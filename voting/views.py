    ...
    for position in positions:
        ...
        for candidate in candidates:
            if position.max_vote > 1:
                instruction = "Vous pouvez sélectionner jusqu'à " + \
                    str(position.max_vote) + " candidats"
                input_box = '<input type="checkbox" value="'+str(candidate.id)+'" class="flat-red ' + \
                    position_name+'" name="' + \
                    position_name+"[]" + '">'
            else:
                instruction = "Sélectionnez uniquement un candidat"
...
def verify(request):
    context = {
        'page_title': 'Vérification par mot de passe'
    }
...
def resend_otp(request):
    ...
    if settings.SEND_OTP:
        if voter.otp_sent >= 3:
            error = True
            response = "Vous avez demandé un mot de passe trois fois. Vous ne pouvez pas le faire à nouveau ! Veuillez entrer l'OTP précédemment envoyé."
        else:
            phone = voter.phone
            ...
            if otp is None:
                ...
                voter.save()
            try:
                msg = "Cher " + str(user) + ", Veuillez utiliser " + \
                    str(otp) + " comme votre mot de passe"
                ...
                if message_is_sent:  # * OTP was sent successfully
                    ...
                    response = "Le mot de passe à usage unique a été envoyé à votre numéro de téléphone. Veuillez le fournir dans le champ ci-dessous."
                else:
                    error = True
                    response = "Le mot de passe non envoyé. Veuillez réessayer."
            except Exception as e:
                response = "Le mot de passe n'a pas pu être envoyé." + str(e)
...
def verify_otp(request):
    ...
    if request.method != 'POST':
        ...
        if otp is None:
            messages.error(request, "Veuillez fournir un mot de passe valide")
        else:
            # Get User OTP
            voter = request.user.voter
            db_otp = voter.otp
            if db_otp != otp:
                messages.error(request, "Le mot de passe fourni n'est pas valide")
            else:
                messages.success(
                    request, "Vous êtes maintenant vérifié. Veuillez voter")
                voter.verified = True
                voter.save()
                error = False
    if error:
...
def preview_vote(request):
    if request.method != 'POST':
        error = True
        response = "Veuillez parcourir correctement le système"
    else:
        output = ""
        ...
        for position in positions:
            ...
            if position.max_vote > 1:
                ...
                if len(form_position) > max_vote:
                    error = True
                    response = "Vous ne pouvez choisir que " + \
                        str(max_vote) + " candidats pour " + position.name
                else:
                    ...
                    for form_candidate_id in form_position:
                        try:
                            ...
                            error = True
                            response = "Veuillez naviguer correctement dans le système"
...
def submit_ballot(request):
    if request.method != 'POST':
        messages.error(request, "Veuillez naviguer correctement dans le système")
        return redirect(reverse('show_ballot'))
    ...
    if len(form.keys()) < 1:
        messages.error(request, "Veuillez sélectionner au moins un candidat")
        return redirect(reverse('show_ballot'))
    positions = Position.objects.all()
    form_count = 0
    for position in positions:
        ...
        if position.max_vote > 1:
            ...
            if form_position is None:
                continue
            if len(form_position) > max_vote:
                messages.error(request, "Vous ne pouvez choisir que " +
                               str(max_vote) + " candidats pour " + position.name)
                return redirect(reverse('show_ballot'))
            else:
                for form_candidate_id in form_position:
                    ...
                    try:
                        ...
                        messages.error(
                            request, "Veuillez naviguer correctement dans le système " + str(e))
                        return redirect(reverse('show_ballot'))
        else:
            this_key = pos
            form_position = form.get(this_key)
    ...
    if (inserted_votes.count() != form_count):
        ...
        messages.error(request, "Veuillez réessayer de voter !")
        return redirect(reverse('show_ballot'))
    else:
        # Update Voter profile to voted
        voter.voted = True
        voter.save()
        messages.success(request, "Merci d’avoir voté")
        return redirect(reverse('voterDashboard'))
